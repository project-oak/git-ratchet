...
# Generate a witness (cosigner) Ed25519 key pair for git-ratchet.
#
# Usage:
#   bazel run //tools:genwitnesskey -- <output-dir> <name>
#
# Creates:
#   <output-dir>/witness-key — witness private key file (Ed25519 cosigner, type 0x04)
#
# The vkey (verifier key) is printed to stdout.
set -euo pipefail

if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <output-dir> <name>" >&2
    echo "" >&2
    echo "  output-dir   Directory to write the witness-key file into" >&2
    echo "  name         Key name (e.g. my-witness)" >&2
    exit 1
fi
...
echo "Witness key written to $KEYFILE"
echo "  vkey: $VKEY"
echo ""
echo "Store the key file as a GitHub Actions secret (WITNESS_SIGNING_KEY)."
echo "The vkey (first line) is the public verifier key for this witness."
