...
// Usage:
//
//	echo "$REQUEST_BODY" | witness-cosign \
...
func main() {
	...
	keyEnv := flag.String("key-env", "", "Environment variable name containing the witness signing key")
	...
	if *keyEnv == "" {
		fmt.Fprintln(os.Stderr, "error: --key-env is required")
		os.Exit(1)
	}

	// Read witness key from environment variable.
	keyData := os.Getenv(*keyEnv)
	if keyData == "" {
		fmt.Fprintf(os.Stderr, "error: environment variable %s is empty or not set\n", *keyEnv)
		os.Exit(1)
	}
	witnessSigner, err := parseKeyFromString(keyData)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: parsing witness key from %s: %v\n", *keyEnv, err)
		os.Exit(1)
	}

	// Read trusted origin vkeys.
	trustedOrigins, err := readCosignOriginsFile(*originVKeysPath)
	if err != nil {
...
// parseKeyFromString parses a witness key from a string in the same format as
// note.ReadKeyFile (line 1: vkey, line 2: base64 seed).
func parseKeyFromString(data string) (*note.Signer, error) {
	lines := strings.SplitN(strings.TrimSpace(data), "\n", 2)
	if len(lines) != 2 {
		return nil, fmt.Errorf("key data must have 2 lines: vkey and base64 seed")
	}

	name, sigType, _, err := note.ParseVKey(strings.TrimSpace(lines[0]))
	if err != nil {
	...
	// For Ed25519, select the correct type byte for cosigner role.
	if sigType != note.MLDSA44 {
		sigType = note.Ed25519Cosigner
	}
	return note.NewSigner(name, seed, sigType, note.RoleCosigner)
}

// readCosignOriginsFile reads trusted origin vkeys from a file, one per line.
// Lines starting with # and blank lines are ignored.
func readCosignOriginsFile(path string) (map[string]cosignOriginKey, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
